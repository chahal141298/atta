        ...
        do {
            if let bundlePath = Bundle.main.path(forResource: "awsconfiguration", ofType: "json"),
               let jsonData = try String(contentsOfFile: bundlePath).data(using: .utf8) {
                ...
                if let webHook = customConfiguration["WebHook"] as? String {
                    self.webHook = webHook
                }
                if let mqttURL = customConfiguration["MQTT"] as? String {
                    self.mqttURL = mqttURL
                }
                if let policyName = customConfiguration["PolicyName"] as? String {
...
/* MQTT URL
 Dev MQTT: wss://a2ojrswqo2dift-ats.iot.us-east-1.amazonaws.com/mqtt
 QA MQTT: wss://amehjudt7qyck-ats.iot.us-east-1.amazonaws.com/mqtt
 Prod MQTT: wss://a17d52y7oz7e9d-ats.iot.us-east-1.amazonaws.com/mqtt
 */
