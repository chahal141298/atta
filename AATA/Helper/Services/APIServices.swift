...
class APIServices: NSObject {
    ...
    func getWeatherData(_ queryString: String, _ completion: @escaping([String: Any], Bool) -> Void) {
        ...
        if self.isCalledInLast12Hours(){
            let cacheResponse = PINCache.shared.object(forKey: "weatherData") as! [String: Any]
            completion(cacheResponse,true)
        }else{
            ...
            NetworkManager.sharedInstance.requestFor(url: APIList.Main.glassStormWeather + queryString, param: nil, httpMethod: .get, includeHeader: false, encodingType: JSONEncoding.default, customHeader: headerParam) { (response, statusCode)  in
                ...
                PINCache.shared.setObject(response, forKey: "weatherData")
                completion(response, true)
            } failure: { (error, statusCode)  in
                self.errorHandler(error)
                completion([:] , false)
    ...
    func getNotesAPI(_ completion: @escaping([Notes], Bool) -> Void) {
        NetworkManager.sharedInstance.requestFor(url: APIList.Main.getNotes + "?" + "clientId=\(NetworkConfiguration.shared.clientId)", param: nil, httpMethod: .get, includeHeader: true, encodingType: JSONEncoding.default, success: { (response, statusCode) in
            guard let status = response["status"] as? String, status.trim().lowercased() == "success" else {
                self.errorHandler(response)
                completion([] , false)
                return
    ...
    func createEndDevice(param: [String: Any], _ completion: @escaping([String: Any], Bool) -> Void){
        NetworkManager.sharedInstance.requestFor(url: APIList.Main.createEndDevice, param: param, httpMethod: .post, includeHeader: true, encodingType: JSONEncoding.default, success: { (response, statusCode) in
            guard let status = response["status"] as? String, status.trim().lowercased() == "success" else {
                self.errorHandler(response)
                completion(response , false)
                return
